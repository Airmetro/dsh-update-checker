...
import { readFile, writeFile, mkdir, copyFile, cp, readdir, realpath, lstat, rm, mkdtemp, rename, appendFile, symlink } from "node:fs/promises";
...
/**
 * Resolve the directory whose children are the profiles.
 *
 * Two layouts exist in the wild: a shared `…/profiles/node_modules` every
 * profile loads from, and a per-profile `…/profiles/<name>/node_modules`. Only
 * the shared one puts the profiles root directly above `node_modules`; in the
 * per-profile layout the owner of `node_modules` *is* a profile directory.
 * Reading the root off a single level makes `profileDirOf` and
 * `findDeclaringProfiles` miss every manifest, so an update is never written
 * back to `package.json` and the next `pnpm install` silently restores the old
 * version.
 * @param profileNodeModules - resolved profile node_modules directory.
 * @returns the directory whose children are profiles.
 */
function pickProfilesRoot(profileNodeModules) {
  const owner = dirname(profileNodeModules);
  if (basename(owner).toLowerCase() === "profiles") return owner;
  const grand = dirname(owner);
  if (basename(grand).toLowerCase() === "profiles") return grand;
  return owner;
}


/**
 * Resolve the Harness home for state, backups and logs.
 *
 * An install whose profile layer really sits in a `…/profiles/node_modules` tree
 * already tells us where the home is (`DSH_UC_PROFILE_NODE_MODULES` overrides and
 * ordinary installs both land here), so that derivation wins. Only when the
 * layout does *not* look like a Harness home — this package installed somewhere
 * else entirely, e.g. a deployment root's `node_modules` — do we fall back to the
 * rules dsh itself uses: `DSH_HOME`, then `~/.dsh`. Deriving a home from a
 * non-profile parent would otherwise scatter state next to the program files.
 * @param envHome - raw `process.env.DSH_HOME` value.
 * @param profileNodeModules - resolved profile node_modules directory.
 * @returns the absolute Harness home.
 */
function pickDshHome(envHome, profileNodeModules) {
  const profilesRoot = pickProfilesRoot(profileNodeModules);
  if (basename(profilesRoot).toLowerCase() === "profiles") return dirname(profilesRoot);
  const raw = typeof envHome === "string" ? envHome.trim() : "";
...
const PROFILES_ROOT = pickProfilesRoot(PROFILE_NODE_MODULES);

const DSH_HOME = pickDshHome(process.env.DSH_HOME, PROFILE_NODE_MODULES);
...
const UPDATE_LOCK_FILE = join(DSH_HOME, "dsh-update-checker-update.lock");

const PROGRESS_STALE_MS = 10 * 60 * 1000;

const LOCK_GRACE_MS = 120000;

const UC_TMP_PREFIX = ".dsh-uc-";
...
/**
 * Result of the last `ensurePluginMount()` run, so `/status.json` can report
 * whether this plugin is actually mounted the way the running dsh resolves
 * modules. `null` until the first run finishes.
 */
let lastMountReport = null;
...
async function findCompositionFile() {
  ...
  if (running) {
    const own = join(running, "cordis.patch.yml");
    try {
...
function npmCliCandidates(exeDir, opts = {}) {
  const platform = opts.platform || process.platform;
  const major = opts.nodeMajor || String(process.versions.node || "").split(".")[0];
  const prefix = opts.prefix || process.env.npm_config_prefix || process.env.NPM_CONFIG_PREFIX || "";
  const dirs = [
    join(exeDir, "node_modules"),
    join(exeDir, "..", "node_modules"),
    join(exeDir, "..", "lib", "node_modules"),
    join(exeDir, "..", "libexec", "lib", "node_modules"),
  ];
  if (major) {
    dirs.push(join(exeDir, "..", "lib", `node_modules_${major}`));
    dirs.push(join(exeDir, "..", `node_modules_${major}`));
  }
  if (platform !== "win32") {
    for (const base of ["/usr/lib", "/usr/local/lib", "/opt/homebrew/lib", "/usr/local/opt/npm/lib"]) {
      dirs.push(join(base, "node_modules"));
      if (major) dirs.push(join(base, `node_modules_${major}`));
    }
  }
  const out = dirs.map((d) => join(d, "npm", "bin", "npm-cli.js"));
  if (prefix) {
    out.push(join(prefix, "node_modules", "npm", "bin", "npm-cli.js"));
    out.push(join(prefix, "lib", "node_modules", "npm", "bin", "npm-cli.js"));
    if (major) out.push(join(prefix, "lib", `node_modules_${major}`, "npm", "bin", "npm-cli.js"));
  }
  const seen = new Set();
  return out.filter((p) => (seen.has(p) ? false : (seen.add(p), true)));
}

function npmShimCandidates(exeDir, env = process.env, platform = process.platform) {
  const win = platform === "win32";
  const out = [];
  for (const d of String(env.PATH || "").split(delimiter).filter(Boolean)) {
    out.push(join(d, win ? "npm.cmd" : "npm"));
  }
  out.push(join(exeDir, win ? "npm.cmd" : "npm"));
  const seen = new Set();
  return out.filter((p) => (seen.has(p) ? false : (seen.add(p), true)));
}

function npmCliFromShim(shimPath) {
  const candidates = [
    join(dirname(shimPath), "node_modules", "npm", "bin", "npm-cli.js"),
    join(dirname(shimPath), "..", "lib", "node_modules", "npm", "bin", "npm-cli.js"),
  ];
  const major = String(process.versions.node || "").split(".")[0];
  if (major) candidates.push(join(dirname(shimPath), "..", "lib", `node_modules_${major}`, "npm", "bin", "npm-cli.js"));
...
function realNodeFromExecPath(nodePath) {
  try {
    const isBatch = process.platform === "win32" && /\.(cmd|bat)$/i.test(nodePath);
    const r = isBatch
      ? spawnSync(process.env.ComSpec || "cmd.exe", ["/c", nodePath, "-p", "process.execPath"], {
          encoding: "utf8",
          timeout: 8000,
          windowsHide: true,
        })
      : spawnSync(nodePath, ["-p", "process.execPath"], { encoding: "utf8", timeout: 8000, windowsHide: true });
    if (r.status === 0 && r.stdout) {
      const real = String(r.stdout).trim();
      if (real && real !== nodePath && existsSync(real)) return real;
    }
  } catch {
...
function resolveRealNode(candidate) {
  const exeDir = dirname(candidate);
  const hasNpmBeside = npmCliCandidates(exeDir).some((p) => existsSync(p));
  if (!hasNpmBeside) {
    const real = realNodeFromExecPath(candidate);
    if (real) return real;
  }
  return candidate;
}
...
function resolveNodeExe() {
  ...
  if (override && existsSync(override)) {
    nodeExeCache = resolveRealNode(override);
    return nodeExeCache;
  }
  for (const c of buildNodeExeCandidates(process.execPath, process.env)) {
    try {
      if (existsSync(c)) {
        nodeExeCache = resolveRealNode(c);
        return nodeExeCache;
      }
    } catch {
  ...
  for (const d of String(process.env.PATH || "").split(delimiter).filter(Boolean)) {
    for (const nm of names) {
      ...
      try {
        if (existsSync(p)) {
          nodeExeCache = resolveRealNode(p);
          return nodeExeCache;
        }
      } catch {
...
function locateNpmCli(exeDir, env = process.env) {
  const hit = npmCliCandidates(exeDir).find((p) => existsSync(p));
  if (hit) return hit;
  for (const shim of npmShimCandidates(exeDir, env)) {
    try {
      if (!existsSync(shim)) continue;
    } catch {
      continue;
    }
    const viaShim = npmCliFromShim(shim);
    if (viaShim) return viaShim;
    try {
      const target = realpathSync(shim);
      const viaReal = npmCliFromShim(target);
  ...
  const err = new Error(
    `npm-cli.js not found beside Node (${exeDir}). ` +
      `Looked beside the binary, in the distro layout (node_modules, node_modules_<major>, /usr/lib, /usr/local/lib, /opt/homebrew/lib), ` +
      `and through every npm on PATH. If your Node is managed by mise/asdf/nvm, set DSH_UC_NODE_EXE to the real Node binary ` +
      `(e.g. 'mise which node'); in a packaged desktop app there is no npm at all — install Node/npm or set DSH_UC_NODE_EXE.`
  );
  err.code = "ENPMCLI";
  throw err;
}
...
function getNpmCli() {
  ...
  npmCliCache = locateNpmCli(exeDir);
  return npmCliCache;
}

function safeGetNpmCli() {
  try {
    return getNpmCli();
  } catch {
    return null;
...
/**
 * Release channel a version belongs to: its prerelease identifier
 * (`alpha` / `beta` / `rc` / …) or `stable`. Used to tell "a newer build on the
 * channel I already follow" apart from "a different, less stable channel".
 * @param v - version string.
 * @returns the channel label, or `unknown` when the version cannot be parsed.
 */
function releaseChannel(v) {
  const p = parseVersion(v);
  if (!p) return "unknown";
  if (p.pre.length === 0) return "stable";
  const identifier = String(p.pre[0] || "").trim().toLowerCase();
  return identifier || "unknown";
}

/**
 * Decide whether installing `targetVersion` would move this deployment into a
 * different release channel. DeepSeek Harness publishes prereleases only
 * (`latest` is itself an rc), so the older "does a stable version exist"
 * question never blocked anything: an `allowPrerelease: false` host was still
 * promoted from `rc` to `alpha`. The channel the installed version already
 * follows is what matters, so same-channel upgrades stay allowed.
 * @param installedVersion - version currently deployed.
 * @param targetVersion - version the update would install.
 * @param allowPrerelease - the user's explicit opt-in.
 * @returns `{ blocked, code, from, to }`.
 */
function prereleaseGate(installedVersion, targetVersion, allowPrerelease) {
  const from = releaseChannel(installedVersion);
  const to = releaseChannel(targetVersion);
  if (allowPrerelease === true) return { blocked: false, code: null, from, to };
  if (to === "stable" || to === "unknown" || from === "unknown") return { blocked: false, code: null, from, to };
  if (from === to) return { blocked: false, code: null, from, to };
  return { blocked: true, code: "E_PRERELEASE", from, to };
}
...
function isNpxCacheRoot(root) {
  return /(^|[\\/])_npx([\\/]|$)/i.test(String(root || ""));
}
...
async function executeMainUpdate({
  ...
  try {
    ...
    await step("install", "正在安装新版本…", packageProgressPercent(0, totalPackages) ?? 10);
    const spec = `${PACKAGE}@${targetVersion}`;
    const { args, type } = buildNpmInstallArgs(deployRoot, spec);
    args.push("--loglevel=http");
    const lockPkgCount = await countLockfilePackages(deployRoot);
    const totalPackages = lockPkgCount > 0 ? lockPkgCount : null;
    let installedOutput = "";
    let lastInstallPercent = packageProgressPercent(0, totalPackages) ?? 10;
    try {
      const { stdout, stderr } = await runNpmProgress(
        args,
        { cwd: deployRoot, timeout: installTimeoutMs },
        (p) => {
          const fetched = totalPackages ? Math.min(p.tarballs || 0, totalPackages) : (p.tarballs || 0);
          const percent = packageProgressPercent(fetched, totalPackages) ?? lastInstallPercent;
          lastInstallPercent = Math.max(lastInstallPercent, percent);
          step("install", "正在安装新版本（约 2-4 分钟，可安全离开此页面）…", lastInstallPercent, {
            detail: totalPackages
              ? `已下载 ${fetched}/${totalPackages} 个包（${lastInstallPercent}%）`
              : `已下载 ${fetched} 个包`,
            count: { done: fetched, total: totalPackages },
          });
        }
      );
...
function buildPortProbe(platform, port) {
  ...
  return { file: "ss", args: ["-H", "-tlnp", `sport = :${port}`] };
}

function buildPortProbeFallback(platform, port) {
  if (platform === "win32") return null;
  return { file: "lsof", args: [`-tiTCP:${port}`, "-sTCP:LISTEN"] };
}

function runProbeCommand(probe, timeoutMs = 8000) {
  return new Promise((resolve) => {
    let child = null;
    try {
      child = spawn(probe.file, probe.args, {
        windowsHide: true,
...
async function collectPortPids(port, platform = process.platform) {
  const probes = [buildPortProbe(platform, port), buildPortProbeFallback(platform, port)].filter(Boolean);
  for (const probe of probes) {
    const pids = parsePidList(await runProbeCommand(probe));
    if (pids.length) return pids;
  }
  return [];
}

function probePortOpen(port, host = "127.0.0.1", timeoutMs = 1200) {
  return new Promise((resolve) => {
...
async function isServicePid(pid, platform = process.platform) {
  if (platform === "win32") return true;
  const cmd = await procCmdline(pid);
  if (!cmd) return true;
  return /node/i.test(cmd) || /dsh/i.test(cmd);
}

async function servicePidsOnPort(port, platform = process.platform) {
  const pids = await collectPortPids(port, platform);
  const out = [];
  for (const pid of pids) {
    if (pid === process.pid) continue;
    if (await isServicePid(pid, platform)) out.push(pid);
  }
  return out;
}

async function portAlive(port, platform = process.platform) {
  if (await probePortOpen(port)) return true;
  return (await collectPortPids(port, platform)).length > 0;
}

function killPid(pid, platform = process.platform, opts = {}) {
  if (platform === "win32") {
    const args = opts.tree ? ["/PID", String(pid), "/T", "/F"] : ["/PID", String(pid), "/F"];
    try {
      const c = spawn(TASKKILL_EXE, args, { windowsHide: true, stdio: "ignore" });
      c.on("error", () => {});
      return true;
    } catch {
      return false;
  ...
  try {
    process.kill(pid, opts.signal || "SIGKILL");
    return true;
  } catch (err) {
...
const LAUNCHER_ENV_KEYS = ["DSH_UC_LAUNCHER", "DSH_RESTART_LAUNCHER"];
const LAUNCHER_NAMES_WIN = ["DeepSeek Harness.cmd", "deepseek-harness.cmd", "start-deepseek-harness.cmd", "start-dsh.cmd", "dsh.cmd"];
const LAUNCHER_NAMES_POSIX = ["DeepSeek Harness.sh", "deepseek-harness.sh", "start-dsh.sh", "dsh-web.sh"];

function launcherCandidates(root, env = process.env, platform = process.platform) {
  const out = [];
  ...
  if (root) {
    for (const name of platform === "win32" ? LAUNCHER_NAMES_WIN : LAUNCHER_NAMES_POSIX) out.push(join(root, name));
  }
  const seen = new Set();
  return out.filter((p) => {
...
async function findLauncherFile(root, env = process.env, platform = process.platform) {
  for (const candidate of launcherCandidates(root, env, platform)) {
    try {
...
function buildServiceRelaunch(platform, { deployRoot, nodeExe, launcher, entry }) {
  const cwd = deployRoot || (launcher ? dirname(launcher) : undefined);
  if (launcher) {
    return platform === "win32"
      ? { file: process.env.ComSpec || "cmd.exe", args: ["/c", launcher], cwd, detached: true, windowsHide: false, method: "launcher", launcher }
      : { file: launcher, args: [], cwd, detached: true, windowsHide: false, method: "launcher", launcher };
  }
  if (entry) {
    const shim = platform === "win32" && typeof nodeExe === "string" && /\.(cmd|bat)$/i.test(nodeExe);
    return shim
      ? { file: process.env.ComSpec || "cmd.exe", args: ["/c", nodeExe, entry, "web"], cwd, detached: true, windowsHide: false, method: "node-shim" }
      : { file: nodeExe, args: [entry, "web"], cwd, detached: true, windowsHide: false, method: "node" };
  }
  return null;
}

function countNpmTarballFetches(text) {
  const seen = new Set();
  for (const line of String(text || "").split(/\r?\n/)) {
    const m = /npm http fetch GET (?:200|304)\s+(\S+)/.exec(line);
    if (!m) continue;
    if (!/\.tgz(?:[?#]|$)/.test(m[1])) continue;
    seen.add(m[1]);
  }
  return seen.size;
}

function parseNpmPackageCount(text) {
  let n = 0;
  const re = /(?:added|changed)\s+(\d+)\s+packages?/g;
  let m;
...
function packageProgressPercent(done, total, bounds = {}) {
  const min = Number.isFinite(bounds.min) ? bounds.min : 10;
  const max = Number.isFinite(bounds.max) ? bounds.max : 99;
  const t = Number(total);
  if (!Number.isFinite(t) || t <= 0) return null;
  const ratio = Math.max(0, Math.min(1, Number(done) / t));
  return Math.max(min, Math.min(max, Math.round(ratio * 100)));
}

function runningProfileDir(env = process.env, profilesRoot = PROFILES_ROOT) {
  const explicit = typeof env.DSH_PROFILE_DIR === "string" ? env.DSH_PROFILE_DIR.trim() : "";
  const name = typeof env.DSH_PROFILE === "string" ? env.DSH_PROFILE.trim() : "";
  const candidates = [];
  if (explicit) candidates.push(resolve(explicit));
  if (name && profilesRoot) candidates.push(join(profilesRoot, name));
  const root = profilesRoot ? resolve(profilesRoot) : null;
  for (const dir of candidates) {
    const d = resolve(dir);
    if (root && d !== root && !d.startsWith(root + sep)) continue;
    try {
      if (existsSync(join(d, "package.json")) || existsSync(join(d, "cordis.patch.yml"))) return d;
    } catch {
...
function detectPosixUpdateCaps(platform = process.platform) {
  if (platform === "win32") return { probe: true, systemctl: true };
  return {
    probe: commandExists("ss") || commandExists("lsof"),
    systemctl: commandExists("systemctl"),
  };
}
...
async function stopDshService(deployRoot) {
  ...
  let pids = [];
  try {
    pids = await servicePidsOnPort(port, platform);
  } catch {
    pids = [];
  }
  for (const pid of pids) {
  ...
  while (Date.now() < deadline) {
    still = await portAlive(port, platform);
    if (!still) break;
    for (const pid of await servicePidsOnPort(port, platform).catch(() => [])) {
      killPid(pid, platform);
    }
...
async function startDshService(deployRoot) {
  ...
  const deadline = Date.now() + 20000;
  while (Date.now() < deadline) {
    const nums = await servicePidsOnPort(port, platform);
    if (!nums.length && !(await portAlive(port, platform))) break;
    for (const pid of nums) killPid(pid, platform, { tree: true });
    await new Promise((r) => setTimeout(r, 500));
  }
  const launcherFile = await findLauncherFile(deployRoot);
  const entry = join(deployRoot, "node_modules", "@deepseek-ai", "dsh", "lib", "bin.js");
  const nodeExe = resolveNodeExe();
  let plan = null;
  if (launcherFile) {
    plan = buildServiceRelaunch(platform, { deployRoot, nodeExe, launcher: launcherFile, entry: null });
  } else if (platform === "win32" || existsSync(entry)) {
    plan = buildServiceRelaunch(platform, { deployRoot, nodeExe, launcher: null, entry });
  }
  if (plan) {
    try {
      spawn(plan.file, plan.args, {
        cwd: plan.cwd || deployRoot,
        windowsHide: false,
        detached: true,
        stdio: "ignore",
      }).unref();
      await opsLog({ op: "main-service-relaunch", via: plan.method, launcher: launcherFile || null, visible: true });
    } catch (err) {
      return { ok: false, error: `launcher spawn failed: ${err.message}` };
    }
  } else if (!IS_WIN32 && commandExists("systemctl")) {
    const r = spawnSync("systemctl", ["--user", "restart", "dsh-web.service"], {
      encoding: "utf8",
      timeout: 30000,
    });
    if (r.status !== 0) {
      return { ok: false, error: "no launcher available and systemctl --user restart failed" };
    }
  } else {
    return { ok: false, error: "no launcher available on this host" };
  }
  const deadline2 = Date.now() + 30000;
  let pid = null;
  let up = false;
  while (Date.now() < deadline2) {
    const nums = await collectPortPids(port, platform).catch(() => []);
...
async function healthCheckDsh() {
  ...
  if (kind === "auth-gated") {
    notes.push(`GET / -> ${home.status} (auth-gated; the service is listening but serves no frontend on /)`);
    return { ok: true, problems, notes, error: null };
  }
  if (kind === "bad") {
    problems.push(`GET / -> ${home.status}`);
    return { ok: false, problems, error: problems.join("; ") };
  }
...
/**
 * Create the link dsh expects, preferring a Windows junction: `lstatSync().isSymbolicLink()`
 * is true for one, `statSync()`/`realpathSync()` follow it, and — unlike a POSIX-style
 * symlink on Windows — creating it needs neither Developer Mode nor elevation. `dir` is the
 * POSIX equivalent. Directory symlinks need no target-type probe on either platform.
 */
async function linkBestEffort(src, dst) {
  const preferred = process.platform === "win32" ? "junction" : "dir";
  try {
    await symlink(src, dst, preferred);
    return preferred;
  } catch (err) {
    if (process.platform !== "win32" || preferred !== "junction") throw err;
...
/**
 * Point `dst` at `src` with a link, reclaiming only a path we can prove is worth
 * reclaiming. Colocated with `runSync` because both share one rule: a directory in
 * `profiles/node_modules` that holds a copy of a package dsh manages is a launch
 * failure waiting to happen, while anything of unknown origin is untouchable.
 *
 * dsh's `healProfilesModuleFallback`/`ensureSymlink` adopts symlinks and
 * dsh-managed module proxies only; a REAL directory makes it throw
 * `dsh: <path> exists and is not a symlink or dsh-managed module proxy; remove it
 * so dsh can manage the installation fallback` while composing the profile —
 * before the web server binds. `fs.cp(..., { force: true })` does not error on a
 * same-named directory from another source either: it overwrites the files it
 * brings and leaves every other file behind. Both are why the write path links
 * instead of copying, and why an unexpected occupant is reported rather than moved.
 *
 * @param src - the real directory the link must point at.
 * @param dst - the link path to create/repair.
 * @param name - package name (an `@scope/name` form takes its last segment for the log).
 * @param opts.expectedName - package `name` the current occupant must carry to be reclaimed.
 * @returns `{ ok, skipped?, replaced?, error? }` — never throws.
 */
async function linkPackageIntoProfile(src, dst, name, opts = {}) {
  const { expectedName = null } = opts;
  const label = String(name).split("/").pop();
  try {
    ...
    if (st) {
      ...
      if (rpSrc && rpDst && rpSrc === rpDst) return { ok: true, skipped: "junction" };
      
      
      let reclaimable = st.isSymbolicLink();
      if (!reclaimable && st.isDirectory()) {
        const want = expectedName || src;
        const [gotDst, wantSrc] = await Promise.all([
          readJsonFile(join(dst, "package.json")),
          readJsonFile(join(src, "package.json")),
        ]);
        reclaimable = Boolean(
          gotDst &&
            gotDst.name &&
            (gotDst.name === want ||
              (wantSrc && wantSrc.name && gotDst.name === wantSrc.name))
        );
      }
      if (!reclaimable) {
        return {
          ok: false,
          error: `${dst} exists and is not the deploy copy of ${label}; refusing to replace it`,
        };
      }
      await rm(dst, { recursive: true, force: true });
      await linkBestEffort(src, dst);
      return { ok: true, replaced: st.isSymbolicLink() ? "stale-link" : "real-directory-copy" };
    }
...
async function runSync(deployRoot, todo) {
  ...
  for (const item of todo) {
    ...
    try {
      ...
      const link = await linkPackageIntoProfile(src, dst, item.name);
      if (!link.ok) {
        results.push({ name: item.name, from: item.from, to: item.to, ok: false, error: link.error });
        await opsLog({ op: "profile-sync-link-refused", name: item.name, error: link.error });
        continue;
      }
      results.push({
        name: item.name,
        ...
        to: item.to,
        ok: true,
        ...(link.replaced ? { replaced: link.replaced } : {}),
        ...(link.skipped ? { skipped: link.skipped } : {}),
      });
    } catch (err) {
      results.push({
        name: item.name,
...
async function fetchMainPackument(force) {
  ...
  if (!version) throw new Error("npm registry: no @deepseek-ai/dsh version available");
  const hasStable = Object.keys((doc && doc.versions) || {}).some((v) => !isPrerelease(v));
  const value = {
    version,
    hasStable,
...
async function runCheck(force) {
  ...
  report.layoutBlocked = false;
  report.layoutNote = null;
  try {
    const npxRoot = await findDeployRoot();
    if (npxRoot && isNpxCacheRoot(npxRoot)) {
      report.layoutBlocked = true;
      report.layoutNote = "当前 dsh 通过 npx 缓存运行（_npx），主框架自动更新暂不支持；请改用官方本地部署或 npm i -g @deepseek-ai/dsh";
    }
  } catch {}
  return report;
}
...
async function compositionProfileNodeModules() {
  const running = runningProfileDir();
  if (running) {
    const nm = join(running, "node_modules");
    try {
...
async function scanInstalledPlugins() {
  ...
  for (const root of await findAllPluginRoots()) {
    ...
    for (const e of entries) {
      ...
      if (e.name.startsWith(UC_TMP_PREFIX)) continue;
...
function pickFolderWithDialog(initialPath) {
  if (!IS_WIN32) return Promise.resolve(null);
  return new Promise((resolve, reject) => {
...
async function writeProgress(patch) {
  try {
    progressCache = { running: true, hostPid: process.pid, ...(progressCache || {}), ...patch, at: Date.now() };
...
async function writeProgressRecord(record) {
  try {
    progressCache = record;
    await writeFile(UPDATE_PROGRESS_FILE, JSON.stringify(record, null, 2), "utf8");
    return true;
  } catch {
    return false;
  }
...
function isStaleProgressRecord(p, opts = {}) {
  if (!p || typeof p !== "object" || p.running !== true) return false;
  if (p.stale === true) return true;
  const alive = typeof opts.alive === "function" ? opts.alive : () => false;
  const now = Number.isFinite(opts.now) ? opts.now : Date.now();
  const staleMs = Number.isFinite(opts.staleMs) ? opts.staleMs : PROGRESS_STALE_MS;
  const workerPid = Number(p.workerPid);
  if (Number.isInteger(workerPid) && workerPid > 0) return !alive(workerPid);
  const hostPid = Number(p.hostPid);
  if (Number.isInteger(hostPid) && hostPid > 0) return !alive(hostPid);
  const at = Number(p.at || 0);
  return Boolean(at) && now - at > staleMs;
}

function isStaleProgress(p) {
  return isStaleProgressRecord(p, { alive: pidAlive });
}

function progressWorkerAlive(p) {
  const n = Number(p && p.workerPid);
  return Number.isInteger(n) && n > 0 && pidAlive(n);
}

async function reconcileStaleUpdateState(reason) {
  const p = await readProgress();
  if (!isStaleProgress(p)) return { changed: false };
  const at = Number(p.at || 0) || null;
  const record = {
    ...p,
    at: at || Date.now(),
    running: false,
    ...
    percent: null,
    label: "上次更新已中断",
    error: `上次主程序更新未完成即被中断（服务已重启，后台任务已消失），已清理残留状态；可直接重新点击更新。原因：${reason}`,
    code: "E_INTERRUPTED",
  ...
  delete record.count;
  await writeProgressRecord(record);
  await rm(UPDATE_LOCK_FILE, { force: true }).catch(() => {});
  await opsLog({ op: "main-update-stale-reconciled", reason, phase: p.phase, at });
  return { changed: true, phase: p.phase };
}

async function reconcileStaleUpdateLock() {
  let st = null;
  try {
    st = await lstat(UPDATE_LOCK_FILE);
  } catch {
    return { changed: false, absent: true };
  }
  if (progressWorkerAlive(await readProgress())) return { changed: false, live: true };
  const age = Date.now() - Number(st.mtimeMs || 0);
  if (age < LOCK_GRACE_MS) return { changed: false, fresh: true, age };
  await rm(UPDATE_LOCK_FILE, { force: true }).catch(() => {});
  await opsLog({ op: "update-lock-stale-removed", ageMs: age });
  return { changed: true, age };
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const maybeEmit = (done) => {
      ...
      if (onProgress && (done || now - lastEmit > 200)) {
        lastEmit = now;
        onProgress({ httpCount, tarballs: countNpmTarballFetches(stderr), done: !!done, stderrTail: stderr.slice(-400), stdoutTail: stdout.slice(-400) });
      }
    };
    child.stdout.on("data", (d) => {
...
export async function findDshPackageDir(root, pkgName) {
  const nm = join(root, "node_modules", "@deepseek-ai");
  const cands = [
    join(nm, pkgName),
    join(nm, "dsh", "node_modules", "@deepseek-ai", pkgName),
  ];
  for (const c of cands) {
    try {
      if (await exists(join(c, "package.json"))) {
        try { return await realpath(c); } catch { return c; }
      }
    } catch {}
  }
  return null;
...
export async function listDshPackageDirs(root) {
  const nm = join(root, "node_modules", "@deepseek-ai");
  const dirs = new Map();
  const add = async (base) => {
    try {
      const ents = await readdir(base, { withFileTypes: true });
      for (const e of ents) {
        if (!e.isDirectory()) continue;
        if (e.name === "dsh" || e.name.startsWith("dsh-")) {
          if (!dirs.has(e.name)) dirs.set(e.name, join(base, e.name));
        }
      }
    } catch {}
  };
  await add(nm);
  await add(join(nm, "dsh", "node_modules", "@deepseek-ai"));
  const out = [];
  for (const [name, dir] of dirs) out.push({ name, dir });
  return out;
}

export function looksLikeFileLockError(error) {
  const s = String((error && (error.stderr || error.message)) || error || "").toLowerCase();
  return /ebusy|resource busy|eperm|enotempty|eexist|being used by another process|access is denied|locked/.test(s);
}

export function shouldResetStaleLock(physVer, lockedVer, target) {
  return Boolean(physVer && lockedVer && lockedVer !== physVer && physVer !== target);
}

export async function installWithFileLockRetry(runInstall, opts = {}) {
  const maxAttempts = opts.maxAttempts || 3;
  const keepStopped = opts.keepStopped || (async () => ({ ok: true }));
  const isPortLocked = opts.isPortLocked || (async () => false);
  const sleep = opts.sleep || ((ms) => new Promise((r) => setTimeout(r, ms)));
  const onRetry = opts.onRetry || (async () => {});
  let lastErr = null;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    const guard = await keepStopped();
    if (!guard.ok) {
      const err = new Error(`service could not be kept stopped: ${guard.error}`);
      err.code = "E_STOP";
    ...
    try {
      ...
      lastErr = err;
      const locked = looksLikeFileLockError(err);
      const reoccupied = await isPortLocked();
      if (!locked && !reoccupied) return { ok: false, error: err };
      await onRetry({ attempt, locked, reoccupied, error: String((err && err.message) || err) });
...
async function verifyDeployTree(deployRoot, targetVersion) {
  ...
  let packages = [];
  try {
    packages = await listDshPackageDirs(deployRoot);
  } catch {
    problems.push(`@deepseek-ai dir missing`);
    return { ok: false, problems };
  }
  if (packages.length === 0) {
    problems.push(`@deepseek-ai dir missing: ${join(deployRoot, "node_modules", "@deepseek-ai")}`);
    return { ok: false, problems };
  }
  ...
  for (const p of packages) {
    ...
    const pj = join(p.dir, "package.json");
    try {
      const pkg = JSON.parse(await readFile(pj, "utf8"));
      if (pkg.version !== targetVersion) {
        problems.push(`${p.name} version ${pkg.version || "?"} != target ${targetVersion}`);
      }
    } catch {
      problems.push(`${p.name} package.json missing/unreadable`);
    }
  }
  
  const fwDir = await findDshPackageDir(deployRoot, "dsh-web-frontend");
  if (!fwDir) {
    problems.push("dsh-web-frontend not found (top-level or nested)");
  } else {
    const dist = join(fwDir, "dist");
    try {
  ...
  for (const p of packages) {
    ...
    if (!entry) problems.push(`${p.name} has lib/ but no entry file (empty shell)`);
  }
...
async function backupAndReplace(dst, src, filter, manifestSpec) {
  ...
  const swap = await swapDirectoryInPlace(dst, src, filter);
  return backupDir;
}

async function swapDirectoryInPlace(dst, src, filter) {
  const parent = dirname(dst);
  await mkdir(parent, { recursive: true }).catch(() => {});
  const stamp = `${Date.now()}-${process.pid}`;
  const stagingDir = join(parent, `${UC_TMP_PREFIX}staging-${basename(dst)}-${stamp}`);
  const trashDir = join(parent, `${UC_TMP_PREFIX}trash-${basename(dst)}-${stamp}`);
  await sweepUpdateTempDirs(parent, basename(dst));
  await rm(stagingDir, { recursive: true, force: true }).catch(() => {});
  try {
    await cp(src, stagingDir, { recursive: true, force: true, ...(filter ? { filter } : {}) });
  } catch (err) {
    await rm(stagingDir, { recursive: true, force: true }).catch(() => {});
    throw err;
  }
  ...
  try {
    if (await exists(dst)) {
      await rename(dst, trashDir);
      movedAside = true;
    }
    await rename(stagingDir, dst);
  } catch (err) {
    await rm(stagingDir, { recursive: true, force: true }).catch(() => {});
    if (movedAside) {
      try {
        await rename(trashDir, dst);
      } catch (restoreErr) {
        err.message = `${err.message} — rollback of the original directory ALSO FAILED (${restoreErr.message}); original content is kept at ${trashDir}`;
      }
    }
    throw err;
  }
  let trashRemoved = true;
  if (movedAside) {
    try {
      ...
      trashRemoved = false;
      await opsLog({
        op: "plugin-update-trash-kept",
        dir: trashDir,
        note: "a locked file blocked deletion; it is removed on a later update",
      });
    }
  }
  return { stagingDir, trashDir, movedAside, trashRemoved };
}

async function sweepUpdateTempDirs(parent, base) {
  let entries = [];
  try {
  ...
  for (const e of entries) {
    if (!e.isDirectory()) continue;
    if (!e.name.startsWith(UC_TMP_PREFIX)) continue;
    if (base && !e.name.includes(`-${base}-`)) continue;
    await rm(join(parent, e.name), { recursive: true, force: true }).catch(() => {});
  }
}
...
async function finalizePluginInstall(target, pkgRoot, depRoot, extra, copyFilter) {
  ...
  let mount = null;
  try {
    mount = await ensurePluginMount();
    lastMountReport = mount;
  } catch (err) {
    mount = { ok: false, errors: [{ step: "mount", error: String((err && err.message) || err) }] };
  }
  return {
    ok: true,
    name: target.name,
...
/**
 * Every profile dsh could be launched with: the directories directly under
 * `PROFILES_ROOT` that carry a `package.json`. A `dsh` key marks a harness
 * profile (that is what dsh's own `initProfile` writes; the `dsh.profile`
 * sub-object is not required, so an unusual but valid manifest still counts).
 * @returns `[{name, dir, pkg, isHarnessProfile}]` — `[]` when the root is unreadable.
 */
async function listProfileDirs() {
  let dirs;
  try {
    dirs = await readdir(PROFILES_ROOT, { withFileTypes: true });
  } catch {
    return [];
  }
  const out = [];
  for (const d of dirs) {
    if (!d.isDirectory()) continue;
    const dir = join(PROFILES_ROOT, d.name);
    const pkg = await readJsonFile(join(dir, "package.json"));
    if (!pkg || typeof pkg !== "object") continue;
    out.push({ name: d.name, dir, pkg, isHarnessProfile: Boolean(pkg.dsh) });
  }
  return out;
}

/**
 * Re-establish this plugin's own mount inside every profile, which is what dsh
 * `0.1.6-alpha.2` requires and what the pre-0.1.6 install recipe no longer
 * provides.
 *
 * `0.1.6-alpha.2` changed the profile module-resolution default from `"link"` to
 * `"runtime"`. Under `"runtime"` the deployment registers
 * `$DSH_HOME/profiles/node_modules` as a *shared managed* directory and excludes
 * it from Node's native resolve, so it can only serve the deployment dependency
 * closure and the selected bundle closure (see `PluginPackages` /
 * `routeScoped` in `@deepseek-ai/dsh-app-boot`). A third-party plugin is in
 * neither set, so a bare `import 'dsh-update-checker'` from the profile's own
 * `cordis.patch.yml` stops resolving and the launch dies with
 * `ERR_MODULE_NOT_FOUND` — with the importer path in the message rewritten, so
 * it falsely points at the profile directory rather than at dsh's fallback base.
 *
 * Two changes fix it, and both are needed:
 *
 * 1. `profiles/<profile>/node_modules/dsh-update-checker` → a link to the real
 *    package in `profiles/node_modules`. This is the decisive one: `routeScoped`
 *    walks Node's search path and stops at the shared directory, so a candidate
 *    *before* it (the profile's own `node_modules`) resolves natively — the
 *    documented "pnpm-managed entries in the profile's `node_modules` resolve
 *    first". dsh only excludes links that point into `.dsh-module-fallback`, and
 *    `ensureProfileSymlink` never replaces an existing pnpm-managed entry.
 *    A JUNCTION, never a copy: the link's real path must stay
 *    `…/profiles/node_modules/…` or `pickDshHome` loses the home and the plugin's
 *    self-location (and with it the sync target for `@deepseek-ai/*`) drifts.
 * 2. The `dependencies` declaration in `profiles/<profile>/package.json`. dsh's
 *    `readProfilePlugins`/`installedProfilePackageNames` and this plugin's own
 *    `findDeclaringProfiles`/`persistPluginUpdate` all use
 *    `hasOwnProperty(deps, name)` to decide which profile owns a plugin, so
 *    without it the plugin reports itself as permanently outdated.
 *
 * Under `"link"` mode (pre-0.1.6 and any future default) no route hook is
 * installed and native resolution reaches the same link, so this is correct for
 * both modes rather than a version-conditional hack.
 *
 * Declarations are only written into profiles that parse as harness profiles,
 * and an existing entry (including a `link:`/`file:` one) is never overwritten —
 * a foreign spec is reported as `foreignDecl`, not silently repaired.
 *
 * @returns a report: `{pluginDir, profiles[], declared[], linked[], removedLegacyLink[], errors[], skipped?}`.
 */
async function ensurePluginMount() {
  const report = {
    pluginDir: null,
    profiles: [],
    declared: [],
  ...
  let realNM = null;
  try {
    realNM = await realpath(PROFILE_NODE_MODULES);
  } catch {
    report.skipped = "profiles/node_modules missing";
    return report;
  }
  const pluginDir = join(realNM, SELF);
  const selfPkg = await readJsonFile(join(pluginDir, "package.json"));
  if (!selfPkg) {
    report.skipped = `${SELF} is not installed in ${realNM}`;
    return report;
  }
  ...
  const spec = derivePersistedSpec(null, selfPkg.version, null) || `^${selfPkg.version}`;
  const profiles = await listProfileDirs();
  if (!profiles.length) report.skipped = "no profile with a package.json";

  for (const p of profiles) {
    const entry = { profile: p.name, linked: false, declared: false };
    report.profiles.push(entry);

    const linkPath = join(p.dir, "node_modules", SELF);
    try {
      const r = await linkPackageIntoProfile(pluginDir, linkPath, SELF, { expectedName: SELF });
      if (!r.ok) {
        entry.error = r.error;
        report.errors.push({ profile: p.name, step: "link", error: r.error });
      } else {
        entry.linked = true;
        if (r.replaced) {
          entry.replaced = r.replaced;
          report.linked.push({ profile: p.name, replaced: r.replaced });
        }
      }
    } catch (err) {
      const msg = String((err && err.message) || err);
      entry.error = msg;
      report.errors.push({ profile: p.name, step: "link", error: msg });
    }

    if (p.isHarnessProfile) {
      
      
      
      
      const section = declaredSection(p.pkg, SELF);
      const current = (p.pkg[section] && p.pkg[section][SELF]) || null;
      if (typeof current === "string") {
        entry.declared = true;
        entry.section = section;
        if (!/^[\^~]/.test(current.trim())) entry.foreignDecl = current;
      } else {
        try {
          p.pkg[section] = { ...(p.pkg[section] || {}), [SELF]: spec };
          await writeFile(
            join(p.dir, "package.json"),
            JSON.stringify(p.pkg, null, 2) + "\n",
            "utf8"
          );
          entry.declared = true;
          entry.added = true;
          entry.section = section;
          report.declared.push({ profile: p.name, spec, section });
        } catch (err) {
          const msg = String((err && err.message) || err);
          entry.error = msg;
          report.errors.push({ profile: p.name, step: "declare", error: msg });
        }
      }
    } else {
      entry.declared = false;
      entry.notAProfile = true;
    }
  }

  
  
  const legacy = join(PROFILE_NODE_MODULES, SELF);
  try {
    const st = await lstat(legacy);
    if (st.isSymbolicLink()) {
      const rp = await realpath(legacy).catch(() => null);
      const pluginReal = await realpath(pluginDir).catch(() => null);
      if (rp && pluginReal && rp !== pluginReal) {
        await rm(legacy, { recursive: false, force: true });
        report.removedLegacyLink.push(legacy);
      }
    }
  } catch {
  ...
  if (report.linked.length || report.declared.length || report.removedLegacyLink.length) {
    await opsLog({
      op: "plugin-self-mount",
      pluginDir,
      linked: report.linked,
      declared: report.declared,
      removedLegacyLink: report.removedLegacyLink,
...
function findPnpm() {
  ...
  for (const c of pnpmCandidates(dirname(resolveNodeExe()), safeGetNpmCli())) {
...
async function rollbackPlugin(id) {
  ...
  return { ok: true, name, installed: ver, backup: id, persisted, mount };
}
...
function mainUpdatePlatformError(platform = process.platform, caps = null) {
  if (platform === "win32") return null;
  const c = caps || detectPosixUpdateCaps(platform);
  if (c.probe) return null;
  return {
    code: "E_PLATFORM_UNSUPPORTED",
    error:
      "One-click core updates on Linux/macOS need `ss` or `lsof` to find the process listening on the service port, and neither is available on this host. Stop DSH, update it manually, then restart DSH.",
  };
}
...
export default {
  ...
  apply(ctx) {
    ...
    ctx.inject(["webServer"], (webCtx) => {
      ...
      reconcileStaleUpdateState("dsh restarted while an update record was still marked running").catch(() => {});
      reconcileStaleUpdateLock().catch(() => {});
      ...
      ensurePluginMount()
        ...
        .catch((err) => {
          lastMountReport = { errors: [{ step: "mount", error: String((err && err.message) || err) }] };
        });
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          ...
          json(res, 200, { ...status, instanceId: INSTANCE_ID, startedAt: INSTANCE_STARTED_AT, mount: lastMountReport });
        },
      });
      ...
      webServer.register({
        kind: "exact",
        path: "/dsh-update-checker/mount.json",
        handler: async (req, res) => {
          if (lastMountReport === null) {
            return json(res, 202, { ok: null, pending: true, note: "mount check still running" });
          }
          return json(res, 200, lastMountReport);
        },
      });
      ...
      webServer.register({
        kind: "exact",
        path: "/dsh-update-checker/mount",
        handler: async (req, res) => {
          const g = await writeGate(req, res);
          if (g.err) return;
          try {
            lastMountReport = await ensurePluginMount();
          } catch (err) {
            lastMountReport = {
              ok: false,
              errors: [{ step: "mount", error: String((err && err.message) || err) }],
            };
          }
          return json(res, lastMountReport.ok === false ? 500 : 200, lastMountReport);
        },
      });
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          const g = await writeGate(req, res);
          if (g.err) return;
          const platformError = mainUpdatePlatformError();
          if (platformError) return json(res, 501, { ok: false, ...platformError });
          const body = g.body;
          const LOCK_FILE = UPDATE_LOCK_FILE;
          if (updateInFlight && Date.now() - updateStartedAt < 600000)
            return json(res, 409, { ok: false, error: "update already running" });
          const lockState = await reconcileStaleUpdateLock();
          if (lockState.changed === false && lockState.live === true)
            return json(res, 409, { ok: false, error: "update already running (background worker alive)" });
          if (lockState.changed === false && lockState.fresh === true)
          ...
          if (isNpxCacheRoot(root)) {
            return json(res, 400, { ok: false, error: "检测到 dsh 通过 npx 缓存运行（_npx），主框架自动更新暂不支持；请改用官方本地部署或 npm i -g @deepseek-ai/dsh", code: "E_NPX_CACHE" });
          }
          if (!(await findDshPackageDir(root, "dsh-web-frontend"))) {
            return json(res, 400, { ok: false, error: "未在部署根发现 dsh-web-frontend（顶层或嵌套均无），该目录可能不是合法的 dsh 部署或已被破坏；请确认 dsh 通过官方本地部署或 npm i -g 安装后重试，而不是装到错误位置再回滚", code: "E_LAYOUT" });
          }
          ...
          try {
            ...
            const setts = await readSettings();
            const installedForGate = await readInstalledVersion().catch(() => null);
            const gate = prereleaseGate(installedForGate, latestInfo.version, setts.allowPrerelease);
            if (gate.blocked) {
              const detail = `${latestInfo.version} 属于 ${gate.to} 频道，当前安装的 ${installedForGate} 属于 ${gate.from} 频道；默认禁止跨频道升级到预发布版，如需跟踪请在设置中开启 allowPrerelease`;
              await writeProgress({ phase: "error", running: false, percent: null, label: "更新失败", error: detail, code: "E_PRERELEASE" });
              await opsLog({ op: "main-update-prerelease-blocked", target: latestInfo.version, from: gate.from, to: gate.to });
            ...
            const env = {
              ...
              DSH_UC_UPDATE_PREV_INSTANCE: INSTANCE_ID,
            };
            ...
            const child = spawn(ws.file, ws.args, {
              stdio: "ignore",
              windowsHide: true,
            ...
            child.on("error", async (err) => {
              ...
              await opsLog({ op: "main-update-spawn-error", error: truncate(detail, 2000) }).catch(() => {});
            });
            child.unref();
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          let p = await readProgress();
          if (!p) return json(res, 404, { ok: false, error: "no update in progress or recorded" });
          if (isStaleProgress(p)) {
            await reconcileStaleUpdateState("progress record read while its owner process is gone");
            p = (await readProgress()) || p;
          }
          json(res, 200, { ...p, instanceId: INSTANCE_ID });
        },
      });
        ...
        handler: async (req, res) => {
          ...
          try {
            const scriptPath = fileURLToPath(
              new URL(IS_WIN32 ? "../scripts/restart-watchdog.ps1" : "../scripts/restart-watchdog.sh", import.meta.url)
            );
            ...
            let child;
            if (IS_WIN32) {
              const inner = `Start-Process -WindowStyle Hidden -FilePath '${POWERSHELL_EXE}' -ArgumentList '-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File','${scriptPath}'`;
              child = spawn(POWERSHELL_EXE, ["-NoProfile", "-NonInteractive", "-Command", inner], {
                stdio: "ignore",
                windowsHide: true,
                env,
              });
...
export {
  ...
  writeExcludedPlugin,
  pidAlive,
  isStaleProgressRecord,
  isStaleProgress,
  classifyHealthStatus,
  reconcileStaleUpdateState,
  reconcileStaleUpdateLock,
  writeProgressRecord,
  readProgress,
  clearProgress,
  writeProgress,
  swapDirectoryInPlace,
  backupAndReplace,
};
