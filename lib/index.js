    ...
    ctx.inject(["webServer"], (webCtx) => {
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            // 两级 spawn 脱钩：本进程 spawn 的 powershell 立即 Start-Process 一个独立孙进程
            // （跑 restart-watchdog.ps1），随后退出；孙进程不再受 dsh web 被 taskkill 牵连，
            // 能可靠完成 kill → relaunch → watchdog。参数经环境变量传递，规避 -Command 中文路径编码问题。
