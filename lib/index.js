...
//   POST /dsh-update-checker/update      — 完整更新主程序：备份 → `npm install -g
//                                          @deepseek-ai/dsh@latest`（全局安装形态）
...
// 主程序更新命令：npm 全局安装（-g）。本机 dsh 是 npm -g 全局安装（全局前缀无 package.json），
// 非 -g 的 `npm install` 会把全局 node_modules 当 extraneous prune 清空（R30 灾难）；
// -g 原位更新 dsh 及其嵌套依赖（dry-run 已验证：零 add/remove，不破坏 profiles 侧 junction）。
// npm 11 必须 --allow-scripts 授权原生依赖包，否则 exit 1。用 process.execPath 定位 npm-cli.js，
// 不依赖 PATH（本机 PATH 有损坏条目）。
...
const UPDATE_COMMAND = `"${process.execPath}" "${NPM_CLI}" install -g ${PACKAGE}@latest --allow-scripts=${ALLOW_SCRIPTS} --no-audit --no-fund`;
...
export function planSyncFromMaps(deploy, profile) {
  const todo = [];
...
// ── GitHub 源辅助：从 package.json repository 解析 owner/repo；查 releases/latest ──
function parseGhRepo(r) {
  if (!r) return null;
  const url = (typeof r === "string" ? r : r.url) || "";
  // 允许仓库名含点号（如 my.plugin），并剥掉可选 .git 后缀；捕获 owner/repo 前两段。
  const m =
...
// 失败抛错（timeout / HTTP xxx / no release），让调用方能区分"访问不了 GitHub"并回退 npm。
async function fetchGitHubLatest(repo, force) {
  ...
  try {
    ...
    const reason = err && (err.name === "TimeoutError" || err.name === "AbortError")
      ? "timeout"
      : (err && err.message ? err.message : "github fetch failed");
...
// 备份旧版 + 替换（junction 防御，安全替换）；返回备份目录
async function backupAndReplace(dst, src) {
  try {
...
// GitHub 安装通道：codeload tarball 下载 + 解压 + 备份替换（GitHub 优先下载源）
async function updatePluginFromGitHub(target, tag) {
  ...
  try {
    ...
    const res = await fetch(url, { headers: { "user-agent": "dsh-update-checker" }, signal: AbortSignal.timeout(120000) });
    if (!res.ok) throw new Error(`GitHub download HTTP ${res.status}`);
    const buf = Buffer.from(await res.arrayBuffer());
    const pkgRoot = extractTarGzToDir(buf, tmp);
    const pkg = JSON.parse(await readFile(join(pkgRoot, "package.json"), "utf8"));
    const backupDir = await backupAndReplace(target.dir, pkgRoot);
...
async function updatePlugin(name) {
  ...
  if (probe.targetSource === "github" && probe.ghTag) {
    return await updatePluginFromGitHub(target, probe.ghTag);
  }
    ...
    ctx.inject(["webServer"], (webCtx) => {
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (body && body.parseError) return json(res, 400, { ok: false, error: "invalid JSON body" });
          if (body && body.confirm !== true) return json(res, 400, { ok: false, error: "confirm required" });
          if (updateInFlight && Date.now() - updateStartedAt < 600000) return json(res, 409, { ok: false, error: "update already running" });
          const root = await findDeployRoot();
      ...
      webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (pluginUpdateInFlight && Date.now() - pluginUpdateStartedAt < 600000) return json(res, 409, { ok: false, error: "plugin update already running" });
...
// 运行时行为与 v1.3.1 完全一致，仅把无副作用的纯函数作为命名导出暴露。
// 注：planSyncFromMaps 用 `export function` 声明，已是命名导出，勿在此重复列出。
export {
  parseVersion,
  compareVersions,
  tagToVersion,
