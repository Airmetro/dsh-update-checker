...
// 部署根目录候选：默认取进程 cwd（start-dsh.cmd 会先 cd 到部署目录再启动 dsh）；
// 若从其它位置启动 dsh，把第二个候选改成实际部署目录即可。
const DEPLOY_ROOT_CANDIDATES = [process.cwd(), "D:\\应用\\DeepSeek-Harness"];

const UPDATE_COMMAND = `npm install ${PACKAGE}@latest`;

// 重启辅助脚本（分离进程运行）：等 2 秒 → 杀掉 3080 端口的监听进程 → 拉起 start-dsh.cmd。
// 用全路径调用 PowerShell 与 taskkill（本机 PATH 有损坏条目）。
const RESTART_HELPER = [
  `Start-Sleep -Seconds 2`,
  `$conn = Get-NetTCPConnection -LocalPort ${WEB_PORT} -State Listen -ErrorAction SilentlyContinue`,
  ...
  `Start-Process -FilePath 'D:\\应用\\DeepSeek-Harness\\start-dsh.cmd' -WorkingDirectory 'D:\\应用\\DeepSeek-Harness' -WindowStyle Hidden`,
].join("; ");
        ...
        handler: async (req, res) => {
          ...
          try {
            const child = spawn(
              ...
              ["-NoProfile", "-NonInteractive", "-Command", RESTART_HELPER],
