...
//                                          终止 3080 端口进程 → 重新拉起 start-dsh.cmd）
...
function buildRestartHelper(deployRoot, port) {
  ...
  return [
    ...
    `$recovered = $false; for ($i = 0; $i -lt 3 -and -not $recovered; $i++) { $deadline = (Get-Date).AddSeconds(30); while ((Get-Date) -lt $deadline) { $c = Get-NetTCPConnection -LocalPort ${port} -State Listen -ErrorAction SilentlyContinue; if ($c) { $recovered = $true; break }; Start-Sleep -Milliseconds 1500 }; if (-not $recovered -and $i -lt 2) { 'watchdog retry ' + ($i + 1) | Out-File -FilePath '${log}' -Append -Encoding utf8; Start-Process -FilePath '${launcher}' -WorkingDirectory '${deployRoot}' -WindowStyle Hidden } }`,
    `if ($recovered) { 'watchdog recovered' | Out-File -FilePath '${log}' -Append -Encoding utf8 } else { 'watchdog FAILED after 3 retries' | Out-File -FilePath '${log}' -Append -Encoding utf8 }`,
...
// 从 npm registry 查询最新版本（带 5 分钟 TTL 缓存，force 时绕过缓存）。
let latestCache = { value: null, at: 0 };
const CHECK_TTL_MS = 5 * 60 * 1000;

async function fetchLatestVersion(force) {
  const now = Date.now();
  if (!force && latestCache.value !== null && now - latestCache.at < CHECK_TTL_MS) {
  ...
  latestCache = { value: data.version, at: Date.now() };
  return data.version;
}
...
async function runCheck(force) {
  ...
  try {
    report.latest = await fetchLatestVersion(force);
  } catch (err) {
