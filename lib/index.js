...
function npmCliCandidates(exeDir) {
  return [
...
function locateNpmCli(exeDir) {
  const hit = npmCliCandidates(exeDir).find((p) => existsSync(p));
  if (hit) return hit;
  const err = new Error(
    `npm-cli.js not found beside Node (${exeDir}). ` +
      `If your Node is managed by mise/asdf/nvm, set DSH_UC_NODE_EXE to the real Node binary (e.g. 'mise which node').`
  );
  err.code = "ENPMCLI";
  throw err;
}
  ...
  try {
    ...
    try {
      const { stdout, stderr } = await runNpmProgress(
        ...
        (p) => {
          const done = totalPackages ? Math.min(p.httpCount, totalPackages) : p.httpCount;
          const percent = totalPackages
            ? Math.min(70, 15 + Math.round((done / totalPackages) * 55))
            : Math.min(70, 15 + Math.floor(p.httpCount / 40));
          step("install", "正在安装新版本（约 2-4 分钟，可安全离开此页面）…", percent, {
            detail: totalPackages ? `已解析 ${done}/${totalPackages} 个包` : `已解析 ${done} 个包`,
            count: { done, total: totalPackages },
          });
        }
      );
...
async function startDshService(deployRoot) {
  ...
  try {
    if (launcherInfo) {
      ...
      const bin = join(deployRoot, "node_modules", "@deepseek-ai", "dsh", "lib", "bin.js");
      if (existsSync(bin)) {
        const c = spawn(resolveNodeExe(), [bin, "web"], {
          cwd: deployRoot,
          detached: true,
          stdio: "ignore",
        });
