// dsh-update-checker — Host half（v1.4.6，已合并 dsh-plugin-checker 的插件更新检测能力）。
// 路由：
...
//   2) <node 目录>/node_modules/pnpm/bin/pnpm.cjs（npm -g 安装形态，需用 node 跑）
//   3) <node 目录>/pnpm（corepack/pnpm 独立安装的 shim）
//   4) <node 目录>/corepack → corepack pnpm
function findPnpm() {
  ...
  const cjs = join(exeDir, "node_modules", "pnpm", "bin", "pnpm.cjs");
  if (existsSync(cjs)) return { cmd: cjs, viaNode: true, corepack: false };
  const shim = join(exeDir, "pnpm");
  if (existsSync(shim)) return { cmd: shim, viaNode: false, corepack: false };
  const corepack = join(exeDir, "corepack");
  if (existsSync(corepack)) return { cmd: corepack, viaNode: false, corepack: true };
  return null;
}
