 ...
 * `PROFILES_ROOT` that carry a `package.json`. A `dsh.profile` key is preferred
 * (that is what dsh's own `initProfile` writes) but not required, so profiles
 * created by an older layout still count.
 */
async function listProfileDirs() {
  ...
  for (const d of dirs) {
    ...
    out.push({ name: d.name, dir, pkg, isHarnessProfile: Boolean(pkg.dsh && pkg.dsh.profile) });
  }
  return out;
}
...
async function ensurePluginMount() {
  ...
  for (const p of profiles) {
    ...
    if (p.isHarnessProfile) {
      ...
      const current = deps[SELF];
      if (typeof current === "string") {
        entry.declared = true;
        if (!/^[\^~]/.test(current.trim())) entry.foreignDecl = current;
      } else {
        try {
          p.pkg.dependencies = { ...deps, [SELF]: spec };
          await writeFile(
          ...
          entry.declared = true;
          entry.added = true;
          report.declared.push({ profile: p.name, spec });
        } catch (err) {
          const msg = String((err && err.message) || err);
          entry.error = msg;
