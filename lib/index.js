// dsh-update-checker — Host half（v1.4.0，已合并 dsh-plugin-checker 的插件更新检测能力）。
// 路由：
...
// 主程序更新前备份目录（lockfile + 版本清单 + backup-meta.json）
const BACKUP_DIR = join(DSH_HOME, "dsh-update-checker-backups");
// 插件旧版备份目录（与备份的插件目录同层，便于恢复）
const PLUGIN_BACKUP_ROOT = join(PROFILE_NODE_MODULES, ".dsh-plugin-backups");
...
function readNpmMajor() {
  try {
    const pj = JSON.parse(
      readFileSync(join(dirname(process.execPath), "node_modules", "npm", "package.json"), "utf8")
    );
...
async function fetchGitHubLatest(repo, force) {
  ...
  try {
    ...
    if (r.status === 403) throw new Error("github rate limited (403)");
...
async function pushUnique(found, seen, pkg, dir) {
  if (!pkg) return;
  let key = dir;
  try {
...
async function scanInstalledPlugins() {
  const found = [];
  const seen = new Set(); // 按实际目录（realpath）去重
    ...
    for (const e of entries) {
      ...
      if (e.name.startsWith("@")) {
        ...
        for (const s of subs) {
          ...
          await pushUnique(found, seen, pkg, dir);
        }
      } else {
...
async function backupAndReplace(dst, src, filter) {
  try {
  ...
  let backupDir = null;
  if (await exists(dst)) {
    backupDir = join(PLUGIN_BACKUP_ROOT, `${basename(dst)}-${Date.now()}`);
    ...
    await writeFile(
      ...
      JSON.stringify({ name: basename(dst), pkgName, original: dst, at: Date.now() }, null, 2),
      "utf8"
    );
...
async function backupDep(name, dst) {
  const root = join(PLUGIN_BACKUP_ROOT, `dep-${name.replace(/[/\\]/g, "-")}-${Date.now()}`);
...
async function finalizePluginInstall(target, pkgRoot, depRoot, extra, copyFilter) {
  ...
  const backupDir = await backupAndReplace(target.dir, pkgRoot, copyFilter);
...
async function updatePluginFromGitHub(target, tag, version) {
  ...
  try {
    ...
    if (r.status !== 200) throw new Error(`GitHub download HTTP ${r.status}`);
    const pkgRoot = extractTarGzToDir(r.buf, tmp);
    ...
    return await finalizePluginInstall(
      ...
      { source: "github", npmOutput, allowScripts },
      filterNoNodeModules
    );
  } finally {
...
async function listMainBackups() {
  try {
    ...
    for (const d of dirs) {
      ...
      let meta = null;
      try {
        meta = JSON.parse(await readFile(join(BACKUP_DIR, d.name, "backup-meta.json"), "utf8"));
      } catch {
        // 老备份无 meta
      }
      out.push({
        id: d.name,
        dir: join(BACKUP_DIR, d.name),
        at: d.name,
...
async function listPluginBackups() {
  try {
    const dirs = await readdir(PLUGIN_BACKUP_ROOT, { withFileTypes: true });
    const out = [];
    for (const d of dirs) {
      if (!d.isDirectory()) continue;
      let info = null;
      try {
        info = JSON.parse(
          await readFile(join(PLUGIN_BACKUP_ROOT, d.name, "backup-info.json"), "utf8")
        );
      } catch {
        // 老备份无 info
      }
...
async function rollbackPlugin(id) {
  ...
  const dir = join(PLUGIN_BACKUP_ROOT, id);
  if (!(await exists(dir))) {
  ...
  let ver = null;
  try {
    ver = JSON.parse(await readFile(join(resolvedOriginal, "package.json"), "utf8")).version;
  } catch {
    // 恢复后无 package.json 也继续（至少目录回来了）
  }
  npmLatestCache.delete(name);
  return { ok: true, name, installed: ver, backup: id };
}
