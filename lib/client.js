...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ZH = {
      ...
      "settings.on": "开",
      "settings.off": "关",
      "settings.allowPre": "跟踪预发布版",
      "settings.allowPreHint": "开启后主框架可升级到 alpha/beta/rc 预发布版（默认关闭，仅跟踪稳定版）",
    ...
    const EN = {
      ...
      "banner.safeNote":
        "Closing this page will not interrupt the update: it keeps running in the background and restarts DSH when done",
      "banner.restarting":
      ...
      "settings.on": "On",
      "settings.off": "Off",
      "settings.allowPre": "Track pre-release builds",
      "settings.allowPreHint": "Allow the main framework to upgrade to alpha/beta/rc pre-releases (off by default; stable only)",
    ...
    const RELOAD_GUARD_KEY = "dsh-update-checker-reloaded-instance";

    const PROGRESS_STALE_MS = 10 * 60 * 1000;

    function isStaleProgress(p) {
      if (!p || p.running !== true) return false;
      if (p.stale === true) return true;
      const at = Number(p.at || 0);
      return Boolean(at) && Date.now() - at > PROGRESS_STALE_MS;
    }

    function isLiveProgress(p) {
      if (!p || p.stale === true) return false;
      if (p.running === true) return !isStaleProgress(p);
      return Boolean(p.phase) && p.phase !== "done" && p.phase !== "error";
    }

    function useInstanceReloadProbe() {
      react.useEffect(() => {
        let stopped = false;
        let seen = null;
        const probe = () => {
          ...
          fetchStatus(STATUS_URL)
            .then((data) => {
              ...
              const id = typeof data.instanceId === "string" && data.instanceId ? data.instanceId : null;
              if (!id) return;
              if (seen === null) {
                seen = id;
                return;
              ...
              let guarded = null;
              try {
                guarded = sessionStorage.getItem(RELOAD_GUARD_KEY);
              } catch {
                guarded = null;
              }
    ...
    function UpdateBanner(props) {
        ...
        return react.createElement(
          ...
          react.createElement(
            ...
            isUpdating && progress
              ...
              : null,
            isUpdating
              ? react.createElement("div", { className: "dsh-update-hint" }, t("banner.safeNote"))
              : null
          )
        );
      }
      ...
      if (d.hasUpdate) {
        ...
        return react.createElement(
          ...
          react.createElement(
            ...
            d.layoutNote
              ? react.createElement("div", { className: "dsh-update-detail" }, d.layoutNote)
              : null,
            react.createElement(
              "div",
              { className: "dsh-update-actions" },
      ...
      const suppressNow = () => {
        postJson(SETTINGS_URL, { suppressUpToDate: true })
          .then((res) => {
    ...
    function PluginBanner(props) {
      ...
      const suppressPlugin = () => {
        postJson(SETTINGS_URL, { suppressPluginBanner: true })
          .then((res) => {
            if (!res || !res.ok) throw new Error((res && res.error) || "suppress failed");
            setDismissed(true);
          })
    ...
    function SettingsSection(props) {
      ...
      const allowPreOn = settings ? settings.allowPrerelease === true : false;
      const toggles = [
        ...
        toggleSwitch(t("settings.allowPre"), allowPreOn, () => saveSetting({ allowPrerelease: !allowPreOn })),
      ];
      const allowPreHint = el("div", { className: "dsh140-mut", style: { width: "100%" } }, t("settings.allowPreHint"));
