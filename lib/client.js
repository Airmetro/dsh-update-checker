// dsh-update-checker — Client half (web module bundle, ModuleLoader format). v1.4.6
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SettingsSection(props) {
      ...
      // v1.4.9：默认下载源下拉（github/npm/smart；仅 npm 与 GitHub 版本一致时生效）
      const sourceRow = settings
        ? el("div", { style: { ...st.row, flexWrap: "wrap" } },
            el("span", { style: st.name }, t("settings.downloadSource")),
            el("select", {
              value: settings.downloadSource || "github",
