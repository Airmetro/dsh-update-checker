// dsh-update-checker — Client half (web module bundle, ModuleLoader format). v1.4.4
// 已合并 dsh-plugin-checker：页面加载同时检查
//   A) 主程序更新（/dsh-update-checker/status.json）→ 主程序横幅（顶部 top:64px）
//   B) 第三方插件更新（/dsh-update-checker/plugins.json）→ 插件横幅（top:144px，错开）
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── 样式注入（幂等；主程序横幅 top:64px，插件横幅 top:144px 避免重叠）──
    if (
      ...
      tag.textContent =
        ...
        ".dsh-plugin-banner{position:fixed;top:144px;left:50%;transform:translateX(-50%);z-index:9998;pointer-events:auto;max-width:min(560px,calc(100vw - 32px));background:var(--dsw-alias-bg-overlay,#1f2937);color:var(--dsw-alias-label-primary,#f3f4f6);border:1px solid var(--dsw-alias-border-l2,#374151);border-radius:10px;padding:12px 16px;box-shadow:0 8px 24px rgba(0,0,0,.35);font-size:13px;line-height:1.5;}" +
    ...
    function SettingsSection(props) {
      ...
      const processNext = () => {
        ...
        postJson(task.type === "main" ? UPDATE_URL : PLUGIN_UPDATE_URL, task.type === "main" ? {} : { name: task.name })
          ...
          .finally(() => { stopPoll(); updatingRef.current = null; setUpdating(null); processNext(); });
      };
