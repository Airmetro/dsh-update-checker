...
- **v1.4.1** — Update pipeline & backup management hardening (verified on this machine):
  - **Main-program update no longer times out / fakes success**: uses an **explicit version** instead of `@latest` (which re-resolves the whole tree — 145s on this machine, easy to time out; explicit versions hit the cache fast path ~1s); works around npm 11's **reify fast-path skip** (npm advances the spec/hidden lockfile to the target but never replaces the actual directory → post-install verification always failed): when the re-read doesn't match, it **renames the target dir and reinstalls** (`forceReifyMain`, ~2s).
  - **Main-program update progress bar**: `/update` writes progress state live (`update-progress.json`); the npm install is parsed package-by-package (`--loglevel=http`, "resolved x/420 packages"), staged (check → dry-run → backup → install → verify → sync → done); shown in both the banner and settings.
  - **Plugin GitHub→npm fallback**: when the GitHub source is a source-only repo (ENOBUILD) / tag mismatch (ETAGMISMATCH) / download too big (ETOOBIG) and the plugin exists on npm, it automatically falls back to the npm channel (dshmarket verified).
  - **Scan dedup by package name**: the same plugin installed in multiple physical locations (top-level + pnpm-hoisted subdirs) is listed once.
  - **Cross-UI update state sync**: banner and settings share the "updating" state (module-level subscription) — reopening settings keeps the progress, and both surfaces stay in sync.
  - **Core banner "Dismiss" now closes**: clicking 知道了 closes this round's banner even when an update exists.
  - **Backup management**: settings gains a "Backup & Restore" card — configurable **backup folder** (default `$DSH_HOME/dsh-update-checker-backups`, plugin backups under `<root>/plugins`, legacy location auto-migrated); **native Windows folder-picker dialog** (TopMost + taskbar, not hidden behind other windows) plus an **"Open folder"** button; **"Clear backup cache"** button (confirm → deletes all backups; rollback buttons disappear accordingly; deleting one plugin's backup hides only its rollback button).
  - **Operations log**: update/rollback/plugin-update/restart key nodes and npm output are appended to `$DSH_HOME/dsh-update-checker-ops.log` for traceability.
