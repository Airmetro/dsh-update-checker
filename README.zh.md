...
- **v1.4.17** — 阻止主框架被自动升进非预期的预发布通道 + 真正的整树快照备份（修复 2026-09-01 事故：`dsh-update-checker` 选中了仍是 `alpha` 的核心更高但插件生态未配齐的 `0.1.2-alpha.3`，且此前没有主框架文件级备份）：
  - **主程序版本改为稳定版优先**：`pickMainLatest` 现在默认返回最高**稳定**版本，绝不再选中 alpha/beta/rc，除非显式开启。此前它取**任意通道**的最高 semver，于是任何已发布的 alpha 都会成为"可更新"目标；状态检查、横幅、`/update` 路由共用此规则。
  - **`allowPrerelease` 开关**（设置页「显示与控制」，默认**关**）：只有开启后检查器才重新跟进预发布版；开关关闭时 `/update` 对预发布目标直接拒绝（`E_PRERELEASE`）。
  - **主框架快照备份**：`backupForUpdate` 现在把 `package.json` + `package-lock.json` **以及** `main-snapshot/node_modules/@deepseek-ai` 整树拷入备份目录，更新失败可从磁盘回滚，而非从 registry 重装旧版（旧备份只记录版本号）。
